...
public:
    virtual float get_chloro(ch_data dat) const
    {
        ...
        { return (float)(dat.mass_of_probe * 1000); };
        ...
        auto add_index = [](float index)
        { return ((float)(100000 / index)); };

        return (float)(add_index(mass_n_correct()) * solvent_correct() * _all.get_chloro(dat) * solv_index());
    }
};
